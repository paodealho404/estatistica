...
pacientes.obitos <- subset(dados.pacientes, 
                           dados.pacientes$situacao_atual == "Óbito" | dados.pacientes$situacao_atual == "óbito por outras causas")
tabela.pacientes.obitos.municipio <- sort(table(pacientes.obitos$municipio_residencia), decreasing=TRUE)
barplot(tabela.pacientesobitos.municipio, 
...
datas.obitos <- cut.Date(as.Date(pacientes.obitos$data_resultado_exame), 
